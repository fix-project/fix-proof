theory eval_tree_coupon
  imports custom_method isabelle_coupon eq_tree_coupon
begin
...
lemma make_some:
  assumes "make_eval_tree_coupon coupons l r = Some res"
  shows "(\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i))) \<and>
         ...
         (\<forall>i. (i < length coupons \<longrightarrow>
           (\<exists>li ri cl cr.
            ...
            is_equal li cl \<and>
            is_equal ri cr))) \<and>
         res = create_coupon Eval l r"
proof -
  have 1: "(\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i)))"
    by (metis assms make_eval_tree_coupon.simps option.distinct(1))
  then have 5: "(\<forall>i. (i < length coupons \<longrightarrow> 
    ...
    using eval_lhs_exist eval_rhs_exist by presburger
  ...
  have 4: "(\<forall>i. (i < length coupons \<longrightarrow>
         (case (get_tree_data_api l i, get_tree_data_api r i, get_coupon_lhs (coupons ! i), get_coupon_rhs (coupons ! i)) of
           ...
           | _ \<Rightarrow> False)))"
    by (metis (no_types, lifting) assms make_eval_tree_coupon.simps option.distinct(1))
  ...
  show ?thesis
    using 1 2 3 5
    by (metis (no_types, lifting) assms make_eval_tree_coupon.simps option.discI option.sel)
qed

lemma make_some_rev:
  assumes "(\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i))) \<and>
         get_tree_size_api l = Some (length coupons) \<and>
         get_tree_size_api r = Some (length coupons) \<and>
         (\<forall>i. (i < length coupons \<longrightarrow>
           (\<exists>li ri cl cr.
            ...
            is_equal li cl \<and>
            is_equal ri cr)))"
  shows "make_eval_tree_coupon coupons l r = Some (create_coupon Eval l r)"
  using assms by fastforce

lemma make_none:
  assumes "make_eval_tree_coupon coupons l r = None"
  shows "(\<exists>i. i < length coupons \<and> \<not>is_eval_coupon (coupons ! i) 
                \<and> (\<forall>j < i. is_eval_coupon (coupons ! j))) \<or>
         ((\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i))) \<and>
...
proof (cases "\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i))")
  case False 
  then have H: "\<exists>i. (i < length coupons \<and> \<not>is_eval_coupon (coupons ! i))" by auto
  have "(\<exists>i. i < length coupons \<and> \<not>is_eval_coupon (coupons ! i) 
                \<and> (\<forall>j < i. is_eval_coupon (coupons ! j)))"
  proof -
    let ?P = "\<lambda>i. i < length coupons \<and> \<not> is_eval_coupon (coupons ! i)"
    let ?i = "LEAST i. ?P i"
    have "?P ?i" using H by (rule LeastI_ex)
    moreover have "\<forall>j<?i. is_eval_coupon (coupons ! j)"
...
next
  case True
  then have T1: "\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i))" .
  ...
  next
    ...
    have "\<not>(\<forall>i. (i < length coupons \<longrightarrow>
           (\<exists>li ri cl cr.
            ...
            is_equal li cl \<and>
            is_equal ri cr)))"
      using T1 T2 T3 assms make_some_rev 
      by force
...
lemma first_loop_internal_good:
  ...
  assumes H2: "is_eval_coupon (coupons ! i)"
...
lemma first_loop_internal_bad:
  assumes H1: "i < length coupons"
  assumes H2: "\<not> is_eval_coupon (coupons ! i)"
  assumes H3: "length coupons < 2^(LENGTH(i32) - 1)"
...
lemma first_loop_good_multiple:
  assumes H1: "\<forall>i. i < x \<longrightarrow> is_eval_coupon (coupons ! i)"
...
next
  case (Suc y)
  then have 1: "y < length coupons" and 2: "is_eval_coupon  (coupons ! y)" 
    by auto
...
lemma first_loop_good:
  assumes H1: "\<forall>i. i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i)"
...
lemma first_loop_bad:
  assumes H1: "(\<exists>i. i < length coupons \<and> \<not>is_eval_coupon (coupons ! i) 
                \<and> (\<forall>j < i. is_eval_coupon (coupons ! j)))"
  assumes H2: "length coupons < 2^(LENGTH(i32) - 1)"
...
proof -
  obtain i where 1: "i < length coupons" 
             and 2: "\<not>is_eval_coupon (coupons ! i)"
             and 3: "(\<forall>j < i. is_eval_coupon (coupons ! j))"
    using H1 by auto
             ...
             [Local_get (Suc 0), Call 16, Local_get 3,
              ...
              b_e.If (Tbv (Some (T_ref T_ext_ref)))
               [EConstNum (ConstInt32 (I32.lift0 0)), Local_set 4,
                ...
                Local_get 0, Local_get (Suc 0), Call 13]
               [Unreachable]]
             [Unreachable]]"
...
lemma E1: "run_iter 
         ...
         (Config 
           ...
           (Frame_context 
              (Redex ((V_ref (ConstRefExtern (to_externref r))) #
                      (V_ref (ConstRefExtern (to_externref l))) #
                      rest_vs)
                     [Invoke func_make_eval_tree_coupon_idx]
                     b_es)
    ...
    apply (simp)
    apply (invoke_coupon_func fuel_idx_def: func_make_eval_tree_coupon_idx_def)
...
"second_be = [Local_get 0, Local_get (Suc 0), Call 13]"
...
= run_iter n
          (Config 
           (Suc n')
           (init\<lparr> tabs := (tabs init)[tab_coupons_idx := ((T_tab \<lparr> l_min = 0, l_max = None\<rparr> T_ext_ref), (map (\<lambda>c. (ConstRefExtern (to_externref c))) coupons))] \<rparr>)
            (Frame_context (Redex (V_ref (ConstRefExtern (to_externref (create_coupon Eval l r))) # rest_vs) [] b_es) lc f  \<lparr>f_locs = locs, f_inst = exp_inst\<rparr>) fcs)"
 using fixpoint_create_eval_coupon_impl
...
lemma make_eval_tree_coupon_run_iter:
  assumes H1: "make_eval_tree_coupon coupons l r = Some res"
  assumes H2: "length coupons < 2^(LENGTH(i32) - 1)"
  shows "run_iter 
  ...
  have 1: "(\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i)))" 
   ...
   and 4: "(\<forall>i. (i < length coupons \<longrightarrow>
           (\<exists>li ri cl cr.
            ...
            is_equal li cl \<and>
            is_equal ri cr)))"
   and 5: "res = create_coupon Eval l r" using make_some[OF H1] by (blast+)
...
lemma make_eval_tree_coupon_run_invoke_none:
  assumes H1: "make_eval_tree_coupon coupons l r = None"
  assumes H2: "length coupons < 2^(LENGTH(i32) - 1)"
  shows "\<exists>cfg msg. run_iter 
          (n + 6 + 37 * (length coupons) + 6 + 17 * (length coupons) + 29)
         (Config 
 ...
 = (cfg, RTrap msg)"
proof -
  have H1: "(\<exists>i. i < length coupons \<and> \<not>is_eval_coupon (coupons ! i) 
                \<and> (\<forall>j < i. is_eval_coupon (coupons ! j))) \<or>
         ((\<forall>i. (i < length coupons \<longrightarrow> is_eval_coupon (coupons ! i))) \<and>
          ((get_tree_size_api l = None \<or> 
            get_tree_size_api r = None \<or> 
  ...
  from H1 show ?thesis
  proof
    assume assm: "\<exists>i<length coupons. \<not> is_eval_coupon (coupons ! i) \<and>
       (\<forall>j<i. is_eval_coupon (coupons ! j))"
  ...
  next
    assume assm: "(\<forall>i<length coupons. is_eval_coupon (coupons ! i)) \<and>
    ((get_tree_size_api l = None \<or>
      get_tree_size_api r = None \<or>
      (\<exists>l'. get_tree_size_api l = Some l' \<and> l' \<noteq> length coupons) \<or>
      (\<exists>r'. get_tree_size_api r = Some r' \<and> r' \<noteq> length coupons)) \<or>
    ...
    then have 1: "(\<forall>i<length coupons. is_eval_coupon (coupons ! i))"
